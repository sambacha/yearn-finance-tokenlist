...
const main = async () => {
  ...
  for (let i = 0; i < newList.tokens.length; i++) {
    let resp = await fetch(
      'https://api.coingecko.com/api/v3/coins/ethereum/contract/' + newList.tokens[i].address,
    );
    ...
    const validDescription =
      !info.description.en || info.description.en.length <= 1000
        ? info.description.en
        : info.description.en.slice(0, 997) + '...';
    ...
    newList.tokens[i] = {
      ...
      extensions: {
        link:
          !info.links.homepage[0] || info.links.homepage[0] === '' ? null : info.links.homepage[0],
  ...
  const newErc20List = {
    ...
    version: nextVersion(
      erc20.version,
      newList.tokens.length > erc20.tokens.length ? VersionUpgrade.MINOR : VersionUpgrade.PATCH,
    ),
  };
  ...
  fs.writeFile(ERC20_LIST_PATH, JSON.stringify(newErc20List), { flag: 'w+' }, function (err) {
    if (err) throw err;

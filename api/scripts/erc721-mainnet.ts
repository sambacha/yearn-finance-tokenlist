...
const erc721: YearnList = require('../../index/mainnet/erc721.json');
...
const main = async () => {
  ...
  const erc721Contracts: string[] = [
    ...new Set([...missingTokens, ...erc721Dump.map((t) => t.contract_address)]),
  ];
  const errors: any = [];
  ...
  const newErc721List = {
    ...
    version: nextVersion(
      erc721.version,
      newYearnList.length > erc721.tokens.length ? VersionUpgrade.MINOR : VersionUpgrade.PATCH,
    ),
  };
  ...
  fs.writeFile(ERC721_LIST_PATH, JSON.stringify(newErc721List), { flag: 'w+' }, function (err) {
    if (err) throw err;
